$(document).ready(function () {
    ...
    $('#modal-add :input[name=valid-from]').datepicker({
        rtl: App.isRTL(),
        orientation: "left",
        autoclose: true,
        format: 'dd.mm.yyyy',
        ...
        var start = $(this).val();
        var end = $('#modal-add :input[name=valid-to]').val();
        var startDate = moment(start, 'DD.MM.YYYY');
        var endDate = moment(end, 'DD.MM.YYYY');
        if (end == '' || (start != 0 && start != '' && moment(endDate).isBefore(startDate))){
            $('#modal-add :input[name=valid-to]').datepicker( "setDate" , new Date(moment(start, 'DD.MM.YYYY')));
        }
    });

    $('#modal-add :input[name=valid-to]').datepicker({
        rtl: App.isRTL(),
        orientation: "left",
        autoclose: true,
        format: 'dd.mm.yyyy',
        ...
        var end = $(this).val();
        var start = $('#modal-add :input[name="valid-from"]').val();
        var startDate = moment(start, 'DD.MM.YYYY');
        var endDate = moment(end, 'DD.MM.YYYY');
        if (start == '' || (start != 0 && start != '' && moment(endDate).isBefore(startDate))){
            $('#modal-add :input[name="valid-from"]').datepicker( "setDate" , new Date(moment(end, 'DD.MM.YYYY')));
        }
    });

    $('#modal-edit :input[name=valid-from]').datepicker({
        rtl: App.isRTL(),
        orientation: "left",
        autoclose: true,
        format: 'dd.mm.yyyy',
        ...
        var start = $(this).val();
        var end = $('#modal-edit :input[name=valid-to]').val();
        var startDate = moment(start, 'DD.MM.YYYY');
        var endDate = moment(end, 'DD.MM.YYYY');
        if (end == '' || (start != 0 && start != '' && moment(endDate).isBefore(startDate))){
            $('#modal-edit :input[name=valid-to]').datepicker( "setDate" , new Date(moment(start, 'DD.MM.YYYY')));
        }
    });

    $('#modal-edit :input[name=valid-to]').datepicker({
        rtl: App.isRTL(),
        orientation: "left",
        autoclose: true,
        format: 'dd.mm.yyyy',
        ...
        var end = $(this).val();
        var start = $('#modal-edit :input[name="valid-from"]').val();
        var startDate = moment(start, 'DD.MM.YYYY');
        var endDate = moment(end, 'DD.MM.YYYY');
        if (start == '' || (start != 0 && start != '' && moment(endDate).isBefore(startDate))){
            $('#modal-edit :input[name="valid-from"]').datepicker( "setDate" , new Date(moment(end, 'DD.MM.YYYY')));
        }
    });
    ...
    $('#modal-add-offer .add-row-offer').on('click', function(e) {
        ...
        var startDate = moment(object.valid_from, 'YYYY-MM-DD');
        var endDate = moment(object.valid_to, 'YYYY-MM-DD');
    ...
    var table = $('#table').on('error.dt', function(e, settings, techNote, message) {
        ...
        columns: [
            ...
            },
            {
                ...
                orderable: false,
                name: 'actions',
                "className": "dt-center nobreakline",
                "data": function ( row, type, val, meta ) {
                    var measures = row.object.measures;
                    var complementState = 'disabled';
                    ...
                    var data =
                        ...
                        '<a class="btn btn-default btn-circle btn-icon-only btn-action dt-complements ' + complementState + '" data-toggle="modal" href="#modal-complements">' +
                        '<i class="icon-briefcase btn-action-icon"></i></a>' +
    ...
    $('#table-offer tbody').on( 'click', '.dt-edit', function (e) {
        ...
        var roomTypes = object.room_types;
        ...
        if (offer.active == 1) {
            ...
            $('#modal-edit-offer :input[name=active]').val(0);
        }
        ...
        if (optionSelected == 'early_booking') {
            if (offer.apply_with_other_offers == 1) {
                ...
                $('#modal-edit-offer :input[name="apply-with-other-offers"]').val(0);
            }
            if (offer.non_refundable == 1) {
                ...
                $('#modal-edit-offer :input[name="non-refundable"]').val(0);
            }
            $('#modal-edit-offer :input[name=booking-date-from]').datepicker("setDate" , new Date(moment(offer.booking_date_from, 'YYYY-MM-DD')));
            $('#modal-edit-offer :input[name=booking-date-to]').datepicker("setDate" , new Date(moment(offer.booking_date_to, 'YYYY-MM-DD')));
    ...
    $('.add-offer').on('click', function (e) {
        ...
        /*var dateFrom = object.valid_from;
        var dateTo = object.valid_to;
    ...
    var tableOffer = $('#table-offer').on('error.dt', function(e, settings, techNote, message) {
        ...
        "ajax": {
            ...
            "type": "POST",
            "data": function ( d ) {
                d.contractId = object == null ? 0 : object.id;
                return d;
            },
            "complete": function(xhr, textStatus) {
                if (xhr.status == '419') {
    ...
    $('table tbody').on( 'click', '.dt-complements', function () {
        var data = table.row( $(this).parents('tr') ).data();
        object = data['object'];
        $('#modal-complements .contract-name').html(object.name);
        tableOffer.draw();
    } );
