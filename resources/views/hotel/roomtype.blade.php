    ...
    $(document).ready(function () {
        ...
        formAdd.validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    "url": "{{ route('hotel.roomtype.create') }}",
                    "type": "POST",
        ...
        formEdit.validate({
            ...
            submitHandler: function (form) {
                var option = $(form).find("button[type=submit]:focus").attr('data');
                $.ajax({
                    "url": "{{ route('hotel.roomtype.update') }}",
                    "type": "POST",
        ...
        $('#table tbody').on( 'click', '.dt-delete', function (e) {
            ...
            $(this).on('confirmed.bs.confirmation', function () {
                if(!sendRequest){
                    $.ajax({
                        url: "{{ route('hotel.roomtype.delete') }}",
                        "type": "POST",
                        "data":  {
                            id: data['id']
                        },
        ...
        $('#table tbody').on( 'click', '.dt-duplicate', function (e) {
            ...
            $(this).on('confirmed.bs.confirmation', function () {
                $.ajax({
                    url: "{{ route('hotel.roomtype.duplicate') }}",
                    "type": "POST",
                    "data":  {
                        id: data['id']
                    },
